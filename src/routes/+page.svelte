<script lang="ts">
  ...
  import { loadWhitelist, isInWhitelist, type WhitelistData } from '$lib/nostr/whitelist';
  // @ts-ignore
  ...
  async function loadWhitelistFromRelay(relay: string) {
    try {
      ...
      whitelist = await loadWhitelist([relay], adminPubkeyHex);
      
      if (whitelist) {
        console.log('✅ Whitelist geladen:', whitelist.pubkeys.length, 'Einträge');
      } else {
        console.warn('⚠️ Keine Whitelist gefunden - Fallback zu Environment Variable');
      }
    } catch (e) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (!isInWhitelist(pubkey, whitelist)) {
