...
<!-- Deal Notification Modal -->
{#if showDealModal && dealModalData}
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  <!-- svelte-ignore a11y-no-static-element-interactions -->
  <div class="modal-overlay" on:click={() => showDealModal = false}>
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div class="deal-modal" on:click|stopPropagation>
      <div class="deal-modal-header">
        {#if dealModalData.type === 'accepted'}
      ...
      <div class="deal-modal-body">
        <p style="white-space: pre-line;">{dealModalData.message}</p>
        
        <div class="room-id-box">
          <strong>Room-ID:</strong>
          <code>{dealModalData.roomId}</code>
        </div>
      ...
      <div class="deal-modal-actions">
        <button 
          ...
          on:click={() => {
            showDealModal = false;
            goto(`/deal/${dealModalData?.roomId}`);
          }}
        >
          🚀 Zum Chat
        </button>
